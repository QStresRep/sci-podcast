...
- 支持 Host:/Scientist: 分角色
- 自动清理；按句分块；逐块合成 MP3；可 ffmpeg 合并
- --only-full-to-docs：只复制 *_full.mp3 到 docs/audio
- ✅ HD 声线（含 ":DragonHD"）不使用 <prosody>/<break>，避免 InvalidSsml
- ✅ 429（TooManyRequests）指数退避重试 + 抖动
- ✅ 全局节流（每块间隔、周期性长休眠），可用环境变量调节
- ✅ 单段也会产 *_full.mp3；打印取消详情；失败/无产物退出 1
"""
...
def synth_ssml(ssml: str, out_path: str, prefer_voice_for_config: str, output_format):
    ...
    max_retries = int(os.getenv("RETRIES", "10"))  # ↑ 重试次数加大
...
def main():
    ...
    # ← 全局节流参数（可通过环境变量调）
    throttle_ms = int(os.getenv("THROTTLE_MS", "3000"))          # 每块之间暂停（默认 3s）
    pause_every = int(os.getenv("CHUNK_PAUSE_EVERY", "5"))       # 每 N 块额外长休眠
    ...
    for fp in files:
        ...
        try:
            ...
            date = raw[1].split(":", 1)[1].strip() if len(raw) > 1 and raw[1].lower().startswith("date:") else datetime.date.today().isoformat()
            body = sanitize_text("\n".join(raw[2:]).strip())
            if len(body) < 20:
